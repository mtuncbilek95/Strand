#include "ModelImporter.h"

#include <Runtime/Resources/Asset/Core/ImporterRegistry.h>
#include <Runtime/Resources/Asset/Model/ModelMetadata.h>

namespace Flax
{
	u32 ModelImporter::AssetTypeId() const
	{
		return 0;
	}

	void ModelImporter::Import(const String& path, AssetMetadata& outMeta)
	{
		Log::Warn(LogType::Asset, "Importing from '{}'", path);

		if (StringHelpers::FileExtension(path).compare(".gltf") != 0)
		{
			Log::Error(LogType::Asset, "Unsupported file extension '{}'", StringHelpers::FileExtension(path));
			return;
		}

		auto tt = dynamic_cast<ModelMetadata*>(&outMeta);

		
		tt->assetId = UuidHelper::GenerateID();
		tt->assetName = StringHelpers::NameWithoutExt(path);

	}

	namespace
	{
		struct ModelImporterRegister
		{
			ModelImporterRegister() { ImporterRegistry::Get().RegisterImporter<ModelImporter>("gltf"); }
		};

		static ModelImporterRegister modelImportRegistered;
	}
}
