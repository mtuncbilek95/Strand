...
namespace Flax
{
	VDescSet::VDescSet(const DescSetProps& desc, VDevice* pDevice) : VObject(pDevice), m_props(desc)
	{
		...
		VDebug::VkAssert(vkAllocateDescriptorSets(m_rootDevice->GetVkDevice(), &info, &m_set), "VDescSet");
	}
	...
	void VDescSet::UpdateData(const SetUpdateProps& desc)
	{
		...
		vkUpdateDescriptorSets(m_rootDevice->GetVkDevice(), static_cast<u32>(desc.entries.size()), writeInformations, 0, nullptr);
	}
}