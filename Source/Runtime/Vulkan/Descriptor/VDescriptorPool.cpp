...
namespace Flax
{
	VDescPool::VDescPool(const DescPoolProps& desc, VDevice* pDevice) : VObject(pDevice), m_props(desc)
	{
		...
		VDebug::VkAssert(vkCreateDescriptorPool(GetRoot()->GetVkDevice(), &poolInfo, nullptr, &m_pool), "VDescPool");
	}
	...
	VDescPool::~VDescPool()
	{
        ...
        {
            vkDestroyDescriptorPool(GetRoot()->GetVkDevice(), m_pool, nullptr);
