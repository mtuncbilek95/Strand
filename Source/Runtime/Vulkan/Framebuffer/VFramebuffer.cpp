...
namespace Flax
{
    ...
    VFramebuffer::~VFramebuffer()
    {
        for (auto& fb : m_framebuffers)
            vkDestroyFramebuffer(m_rootDevice->GetVkDevice(), fb, nullptr);
    ...
    void VFramebuffer::Resize(const Math::Vec3u& size)
    {
        ...
        for (usize i = 0; i < m_props.imageViewsPerFB.size(); ++i)
        {
            ...
            VDebug::VkAssert(vkCreateFramebuffer(m_rootDevice->GetVkDevice(), &fbInfo, nullptr, &m_framebuffers[i]), "VFramebuffer");
        }
    }
}